...
const MyTableColumn = <RecordType extends object = object>(
  props: MyTableColumnProps<RecordType>
) => {
  ...
  const renderContent = (value: any, record: RecordType, index: number) => {
    ...
    if ('datetime' in props) {
      return moment(
        value,
        datetimeFormatMap[typeof datetime === 'string' ? datetime : 'second']
      )
    } else if ('date' in props) {
      return moment(
        value,
        dateFormatMap[typeof date === 'string' ? date : 'day']
      )
    } else if ('time' in props) {
      return moment(
        value,
        timeFormatMap[typeof time === 'string' ? time : 'second']
      )
    }
    ...
    if (dataIndex && options) {
      const data = options.find(
        (item) => item.value === getPathValue(record, dataIndex)
      )
